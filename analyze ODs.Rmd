...
ODs_polysub <- ODs_mut_excl %>%
  mutate(
    rx_opioid_or_heroin_and_coc_or_PS = case_when((heroin == 1 |
                                                     rx_opioids == 1) &
                                                    fentanyl == 0 &
                                                    (cocaine == 1 |
                                                       psychostimulants == 1) ~ 1,
    ...
    all_other_drug_combos = case_when(
      heroin_only == 0 &
        rx_opioids_only == 0 &
        fentanyl_only == 0 &
        cocaine_only == 0 &
        psychostimulants_only &
        rx_opioid_or_heroin_and_coc_or_PS == 0 &
        fentanyl_and_PS == 0 &
        fentanyl_and_coc == 0 ~ 1,
      TRUE ~ 0
    )
...
grant_summary <- ODs_polysub %>% 
  group_by(year) %>% 
  summarize("Heroin only" = sum(heroin_only),
            "Rx opioids only" = sum(rx_opioids_only),
            "Synthetic opioids only" = sum(fentanyl_only),
            "Cocaine only" = sum(cocaine_only),
            "Psychostimulants only" = sum(psychostimulants_only),
            "Rx opioids or heroin with cocaine or psychostimulants" = sum(rx_opioid_or_heroin_and_coc_or_PS),
            "Synthetic opioids and psychostimulants only" = sum(fentanyl_and_PS),
            "Synthetic opioids and cocaine only" = sum(fentanyl_and_coc),
            "All other drug combinations" = sum(all_other_drug_combos))
