...
ODs_polysub <- ODs_mut_excl %>%
  mutate(
    synthetics_with_or_wo_other_opioids = case_when(
      fentanyl == 1 &
        cocaine == 0 &
        psychostimulants == 0 ~ 1,
      TRUE ~ 0
    ...
    cocaine_or_PS_and_rx_or_heroin = case_when(
      (heroin == 1 |
        rx_opioids == 1) &
        fentanyl == 0 &
        (cocaine == 1 |
...
grant_summary <- ODs_polysub %>%
  ...
  summarize(
    ...
    "Synthetic opioids with or without any other opioids" = sum(synthetics_with_or_wo_other_opioids),
    "Synthetic opioids and psychostimulants only" = sum(fentanyl_and_PS),
    "Synthetic opioids and cocaine only" = sum(fentanyl_and_coc),
    "Psychostimulants and cocaine only" = sum(PS_and_coc),
    "Psychostimulants or cocaine with Rx opioids or heroin only" = sum(cocaine_or_PS_and_rx_or_heroin)
  )
